...
export const ARTIST_ENDPOINT = `${LAST_FM.base_url}/2.0/?method=${LAST_FM.methods.user.top_artists}&user=${LAST_FM.user}&limit=10&api_key=${LAST_FM.api_key}&format=${LAST_FM.format.json}`;
export const WEEKLY_ALBUM_CHART = `${LAST_FM.base_url}/2.0/?method=${LAST_FM.methods.user.weekly_album_chart}&user=${LAST_FM.user}&limit=10&api_key=${LAST_FM.api_key}&format=${LAST_FM.format.json}`;
